...
def main():
    global hxloc, planpoint, locFlag
...
#    rospy.Subscriber("/AftMsfFusn", PoseStamped, AfusnCallBack)#/zibet/sensor/gnss/init
    ...
    #plan_track = np.loadtxt(track_url + r'huawei_road.txt')
    ...
    while not rospy.is_shutdown():
        ...
        data_distoend = np.append(data_distoend,[distoend],axis = 0)
        plt.axis([planpoint[0, 0]-20, planpoint[0, 0]+20,
                  planpoint[1, 0]-20, planpoint[1, 0]+20], 'equal')
        plt.legend()
        plt.grid(True)
        ...
        for ii in range(0, 32):
            ...
            data_obstacles_confidence[ii] = np.append(data_obstacles_confidence[ii],[np.array(object_confidence[ii,0])],axis = 0)
